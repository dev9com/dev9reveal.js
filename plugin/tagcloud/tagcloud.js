...
(function(){
    [].forEach.call( document.querySelectorAll('[tagcloud]'), function(cloud) {
        ...
        var headerToIgnore = 'h2';
        var header = cloud.querySelectorAll(headerToIgnore);
        ...
        function formatTags(text) {
            ...
            .map(function(item) {
                ...
                // ignore header h2
                if(item.indexOf(headerToIgnore) > -1) {
                    retValue = item.trim();
                } else {
                    retValue = ( item.indexOf('span') === -1 ) ? '<span>' + item.trim() + '</span> ' : item.trim();
                }
                return retValue;
            })
            .join("");
